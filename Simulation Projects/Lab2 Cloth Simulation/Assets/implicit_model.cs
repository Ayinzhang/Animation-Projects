...
public class implicit_model : MonoBehaviour
{
	...
	void Get_Gradient(Vector3[] X, Vector3[] X_hat, float t, Vector3[] G)
	{
		...
		for(int i=0;i<X.Length;i++)
        {
			G[i] = 1 / t / t * mass * (X[i] - X_hat[i]) - new Vector3(0, mass * g, 0);
        }
		//Spring Force.
		for(int i=0;i<X.Length;i+=2)
        {
			G[i] = G[i] + spring_k * (1 - L[i / 2] / (X[E[i]] - X[E[i + 1]]).magnitude) * (X[i] - X[i + 1]);
			G[i + 1] = G[i + 1] - spring_k * (1 - L[i / 2] / (X[E[i]] - X[E[i + 1]]).magnitude) * (X[i] - X[i + 1]);
		}
	}
